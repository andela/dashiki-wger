...
class GymUserListView(LoginRequiredMixin,
    ...
    def get_queryset(self):
        ...
        for u in Gym.objects.get_members(self.kwargs['pk'], active=self.active).\
...
class GymAddView(WgerFormMixin,
                 LoginRequiredMixin,
...
def gym_new_user_info_export(request):
    ...
    filename = 'User-data-{t.year}-{t.month:02d}-' \
               '{t.day:02d}-{user}.csv'.format(t=today,
                                               user=new_username)
...
class GymUpdateView(WgerFormMixin,
                    LoginRequiredMixin,
                    PermissionRequiredMixin,
                    UpdateView):
    """
...
class GymDeleteView(WgerDeleteMixin,
                    LoginRequiredMixin,
                    PermissionRequiredMixin,
                    DeleteView):
    """
