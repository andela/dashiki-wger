...
def render_workout_day(day, nr_of_weeks=7,
                       images=False, comments=False,
                       only_table=False):
    '''
    ...
    data.append([_('Nr.'), _('Exercise'),
                 _('Reps')] + [_('Weight')] * nr_of_weeks)
    ...
    for set in day['set_list']:
        ...
        for exercise in set['exercise_list']:
            ...
            if comments:
                item_list = [ListItem(Paragraph(
                    i, style=styleSheet["ExerciseComments"]))
            ...
            if images:
                if exercise['obj'].main_image:
                    ...
                    # Make the images somewhat larger
                    # when printing only the workout and not
    ...
    # exercise_start till the end of the data
    # list
...
class WorkoutCalendar(HTMLCalendar):
    ...
    * http://uggedal.com/journal/creating-a-flexible-
    ...
    def formatday(self, day, weekday):
        ...
        # Note: due to circular imports we use can't import the workout
        # session model to access the impression values directly, so they
        # are hard coded here.
        if entry['session']:
            # Bad
    ...
    def day_cell(self, cssclass, body):
        ...
        Renders a day cell
        '''
        return ('<td class="{0}" style="vertical-align:middle;'
                '">{1}</td>'.format(cssclass, body))
