...
class SetAddTestCase(WorkoutManagerAddTestCase):
    ...
    def test_add_set(self, fail=False):
        '''
        Tests adding a set and corresponding settings at
        the same time
        '''
...
class SetDeleteTestCase(WorkoutManagerTestCase):
    ...
    def test_delete_set_other(self):
        '''
        Tests deleting a set from a workout as a
...
class TestSetOrderTestCase(WorkoutManagerTestCase):
    '''
    Tests that the order of the (existing) sets in a workout is
    preservead when adding new ones
    '''
    ...
    def add_set(self, exercises_id):
        ...
        for exercise_id in exercises_id:
            post_data[
                'exercise{0}-TOTAL_FORMS'.format(
                    exercise_id)] = nr_sets
            post_data[
                'exercise{0}-INITIAL_FORMS'.format(exercise_id)] = 0
            post_data[
                'exercise{0}-MAX_NUM_FORMS'.format(exercise_id)] = 1000
            for set_nr in range(0, nr_sets):
                post_data[
                    'exercise{0}-{1}-repetition_unit'.format(
                        exercise_id, set_nr)] = 1
                post_data[
                    'exercise{0}-{1}-weight_unit'.format(
                        exercise_id, set_nr)] = 1
                post_data[
                    'exercise{0}-{1}-reps'.format(exercise_id, set_nr)] = 8
...
class TestSetAddFormset(WorkoutManagerTestCase):
    '''
    Tests the functionality of the formset mini-view that is
    used in the add set page
    '''
    ...
    def get_formset(self):
        ...
        response = self.client.get(
            reverse('manager:set:get-formset',
