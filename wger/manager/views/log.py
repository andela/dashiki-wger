...
# Log functions
# ************************
class WorkoutLogUpdateView(WgerFormMixin,
    ...
    def get_context_data(self, **kwargs):
        context = super(
            WorkoutLogUpdateView, self).get_context_data(**kwargs)
...
class WorkoutLogDeleteView(WgerDeleteMixin,
...
def add(request, pk):
    ...
    # We need several lists here because we need to assign specific
    # form to each
    # exercise: the entries for weight and repetitions have no
    # indicator to which
    ...
    # Define the formset here because now we know the value to
    # pass to 'extra'
    ...
    if request.method == 'POST':
        ...
        # that the form expects a value for the exercise which
        # is not present in
        ...
        for form_id in form_to_exercise:
            if post_copy.get(
                'form-%s-weight' % form_id) or post_copy.get(
                    'form-%s-reps' % form_id):
                post_copy[
                    'form-%s-exercise' % form_id] = form_to_exercise[
                        form_id].id
        ...
        # Initialise the formset with a queryset that won't return any
        # objects
        ...
        # for today, update the current one or create a new one
    ...
    for exercise in exercise_list:
        ...
        exercise_list[exercise]['forms'] = formset[
            form_id_from:form_id_to + 1]
...
class WorkoutLogDetailView(DetailView, LoginRequiredMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        context = super(WorkoutLogDetailView, self).get_context_data(
            **kwargs)
        ...
        for day_list in self.object.canonical_representation['day_list']:
            ...
            for set_list in day_list['set_list']:
                ...
                for exercise_list in set_list['exercise_list']:
                    ...
                    # Filter the logs for user and exclude all units that
                    # are not weight
                    #
                    # TODO: add the repetition_unit to the filter.
                    # For some reason (bug
                    # in django? DB problems?) when adding the
                    # filter there, the
                    # execution time explodes. The weight unit
                    # filter works as
                    # expected. Also, adding the unit IDs to
                    # the exclude list
                    # also has the disadvantage that if new
                    # ones are added in a
