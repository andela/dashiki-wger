...
class Workout(models.Model):
    ...
    def canonical_representation(self):
        ...
        This form makes it easier to cache and use everywhere
        where all or part of a workout structure is needed.
    ...
    def get_current_workout(self, user):
        ...
        try:
            ...
            if schedule.schedulestep_set.count():
                ...
                # Doing it like this
                # is kind of wrong, but lets us continue to the
                # correct place
...
class Schedule(models.Model):
    ...
    name = models.CharField(verbose_name=_('Name'),
                            max_length=100,
                            help_text=_("Name or short description of "
    ...
    is_active = models.BooleanField(
        ...
        help_text=_("Tick the box if you want to mark this "
                    "schedule as your active one (will be "
                    "shown e.g. on your dashboard). All other "
                    "schedules will then be marked as inactive"))
    ...
    is_loop = models.BooleanField(
        ...
        default=False,
        help_text=_("Tick the box if you want to repeat the "
                    "schedules in a loop (i.e. A, B, C, A, B, C, "
                    "and so on)"))
    ...
    def get_end_date(self):
        '''
        Calculates the date when the schedule is
        over or None is the schedule
        is a loop.
        '''
        if self.is_loop:
            return None
...
class ScheduleStep(models.Model):
    ...
    A step is basically a workout a with a
    bit of metadata (next and previous
    steps, duration, etc.)
    '''
...
class Day(models.Model):
    ...
    def get_canonical_representation(self):
        ...
        for set_obj in self.set_set.select_related():
            ...
            # repetitions.
            # If not, just take the smallest number and drop the rest, because
...
class Setting(models.Model):
    ...
    repetition_unit = models.ForeignKey(RepetitionUnit,
                                        ...
                                        default=1)
    '''
    The repetition unit of a set. This can be e.g.
    a repetition, a minute, etc.
    '''
    ...
    def __str__(self):
        ...
        Return a more human-readable representation
        '''
        return u"settings for exercise {0} in set {1}".format(self.exercise.id,
                                                              self.set.id)
...
class WorkoutLog(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        Reset cache
        '''
        reset_workout_log(self.user_id,
                          self.date.year,
                          self.date.month,
                          self.date.day)

        # If the user selected "Until Failure",
        # do only 1 "repetition",
        # everythin else doesn't make sense.
        if self.repetition_unit == 2:
            self.reps = 1
...
class WorkoutSession(models.Model):
    ...
    See note in weight.models.WeightEntry about
     why this is not editable=False
    '''
    ...
    notes = models.TextField(verbose_name=_('Notes'),
                             null=True,
                             blank=True,
                             help_text=_('Any notes you might want'
                                         'to save about this workout '
                                         'session.'))
    '''
