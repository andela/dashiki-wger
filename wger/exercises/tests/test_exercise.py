...
class ExerciseIndexTestCase(WorkoutManagerTestCase):
    ...
    def exercise_index(self, logged_in=True, demo=False, admin=False):
        ...
        if admin:
            self.assertNotContains(response, 'Only registered'
                                             ' users can do this')
...
class ExercisesTestCase(WorkoutManagerTestCase):
    ...
    def add_exercise_user_fail(self):
        '''
        Helper function to test adding exercises by users
    ...
    def add_exercise_success(self, admin=False):
        '''
        Tests adding/editing an exercise with a user with
    ...
    def search_exercise(self, fail=True):
        ...
        self.assertEqual(
            result['suggestions'][0]['data']['image_thumbnail'], None)
...
class ExercisesCacheTestCase(WorkoutManagerTestCase):
    ...
    def test_overview_cache_update(self):
        ...
        self.assertFalse(cache.get(
            get_template_cache_name('muscle-overview-mobile', 2)))
        self.assertFalse(cache.get(
            get_template_cache_name('muscle-overview-search', 2)))
        self.assertFalse(cache.get(
        ...
        if not self.is_mobile:
            ...
            self.assertNotEqual(
                old_exercise_overview_mobile, new_exercise_overview_mobile)
