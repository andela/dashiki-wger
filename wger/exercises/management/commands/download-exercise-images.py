...
class Command(BaseCommand):
    ...
    help = ('Download exercise images from wger.de and update'
            ...
            '           to your local exercises. The exercises'
            ' are identified by\n'
            '           their UUID field, if you manually edited'
            ' or changed it\n'
    ...
    def handle(self, **options):
        ...
        for exercise_json in result['results']:
            ...
            self.stdout.write(u"*** Processing {0} (ID: {1}, UUID: {2})"
                              .format(exercise_name, exercise_id,
                                      exercise_uuid))

            try:
                exercise = Exercise.objects.get(uuid=exercise_uuid)
            except Exercise.DoesNotExist:
                self.stdout.write('    Remote exercise not '
                                  'found in local DB, skipping...')
                continue
            ...
            if images['count']:
                ...
                for image_json in images['results']:
                    ...
                    result = requests.get(thumbnail_api
                                          .format(remote_url, image_id),
                                          headers=headers).json()
                    ...
                    # http://stackoverflow.com/questions/1308386/
                    # programmatically-saving-image-to-
